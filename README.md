...
首次使用先完成上面的安装步骤。之后在 PowerShell 中运行：
...
如果浏览器无法自动打开，后端和前端仍保持运行，可按脚本提示手动访问本地 Web URL。
...
以下命令用于开发调试；普通本地使用可以直接运行 `scripts\start-local.ps1`。
